...
Just download and double-click. This is the NO-ADMIN build: it never asks for
UAC and never re-launches itself. It installs its own dependency (pynput) if
missing, and starts.
...
def _ensure_admin() -> None:
    """No-admin build: never asks for UAC and never re-launches itself.

    Recording and replay still work against every ordinary window. What is
    lost is any window that is itself running as Administrator: Windows
    refuses both the input hook and the injected input across that boundary.
    The badge in the title bar reports the privilege level this copy has.
    """
    return
...
class App:
    ...
    def __init__(self):
        ...
        self.status = "no-admin build - no UAC, cannot reach admin windows"
        self.ticker = ""
    ...
    def _start_player(self):
        ...
        self.player = Player(self.slot().events, self.stop_evt, self.bus, self.echo,
    ...
    # ---- adjusters ---------------------------------------------------------
    def bump_gap(self, d):
        self.gap = max(0.0, min(5.0, round(self.gap + d, 2)))
        self.set_status("gap between runs = %.2fs" % self.gap)
...
class Console:
    ...
    def __init__(self, app, root):
        ...
        tk.Label(cv, text="this session only", bg=T.CARD, fg=T.FAINT_C,
        ...
        self.status_lbl = tk.Label(cv, text="", bg=T.CHROME, fg="#4A3708", anchor="w",
                                   font=(T.MONO, max(6, S(8))))
        self.status_lbl.place(x=S(18), y=S(530), width=S(self.W - 236),
                              height=S(14))
        self.ticker_lbl = tk.Label(cv, text="", bg=T.CHROME, fg="#6A5210", anchor="e",
                                   font=(T.MONO, max(6, S(7))))
        self.ticker_lbl.place(x=S(self.W - 18), y=S(531), width=S(186),
